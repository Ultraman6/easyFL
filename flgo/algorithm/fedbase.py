...
class BasicServer(BasicParty):
    ...
    def __init__(self, option={}):
        ...
        self.sample_record_root = option.get('sample_record_root', None)
        self.sample_record_mode = option.get('sample_record_mode', 'w')
        if self.sample_record_root is not None:
            self.sample_record = {}
            if self.sample_record_mode == 'r' and os.path.exists(self.sample_record_root):
                file_path = os.path.join(self.sample_record_root, 'sample.json')
                with open(file_path, 'r') as f:
                    self.sample_record = json.load(file_path, f)
            elif self.sample_record_mode == 'w':
                os.makedirs(self.sample_record_root, exist_ok=True)
            else:
                raise ValueError("The sample_record_mode should be 'r' or 'w'.")
        else:
    ...
    def run(self):
        ...
        while True:
            ...
            if updated is True or updated is None:
                ...
                # check log interval (eval实现于log_once)
        ...
        if self.sample_record_mode == 'w' and self.sample_record_root is not None:
            file_path = os.path.join(self.sample_record_root, 'sample.json')
    ...
    def sample(self):
        ...
        ```
        """
        if self.sample_record_mode == 'r' and self.current_round in self.sample_record:
            return self.sample_record[self.current_round]
        ...
        # sample clients
            ...
            raise NotImplementedError("The sampling option {} is not supported.".format(self.sample_option))

        if self.sample_record_mode == 'w' and self.sample_record_root is not None:
            self.sample_record[self.current_round] = [int(cid) for cid in selected_clients]
...
class BasicClient(BasicParty):
    ...
    def get_optimizer(self, model):
        return self.calculator.get_optimizer(model, lr=self.learning_rate, weight_decay=self.weight_decay,
