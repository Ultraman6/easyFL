...
def visualize_by_domain_class(generator, partitioner, task_path: str):
    r"""
    Visualize the partitioned multi-domain classification dataset and save the figures

    Args:
        generator (flgo.benchmark.toolkits.BasicTaskGenerator): task generator
        partitioner (flgo.benchmark.toolkits.partition.BasicPartitioner): partitioner
        task_path (str): the path storing the figures
    """
    # 获取全部标签和类别数量
    generator.train_data.set_mode('label')
    all_labels = [d[-1] for d in generator.train_data]
    num_classes = len(set(all_labels))

    # 获取全部领域数量
    generator.train_data.set_mode('domain')
    all_domains = [d[-1] for d in generator.train_data]
    num_domains = len(set(all_domains))

    # 准备颜色
    colors = [key for key in matplotlib.colors.CSS4_COLORS.keys()]
    if len(colors) < max(num_classes, num_domains):
        colors = list(matplotlib.colors._colors_full_map.keys())
    random.shuffle(colors)

    client_height = 1  # 条形图的高度
    data_columns = [len(cidx) for cidx in generator.local_datas]
    n = len(data_columns)
    row_map = {k: i for k, i in zip(np.argsort(data_columns), [_ for _ in range(n)])}
    ...
    for cid, cidxs in enumerate(generator.local_datas):
        ...
        for lbi in range(num_classes):
            plt.fill_between([offset, offset + lb_counter[lbi]], y_bottom, y_top, facecolor=colors[lbi % len(colors)])
            offset += lb_counter[lbi]
    plt.title('Label Heterogeneity')
    plt.xlim(0, max(data_columns))
    plt.ylim(-0.5, n - 0.5)
    plt.ylabel('Client ID')
    plt.xlabel('Number of Samples')
    plt.savefig(os.path.join(task_path, 'label_heterogeneity.png'))
    plt.show()
    ...
    generator.train_data.set_mode('domain')
    for cid, cidxs in enumerate(generator.local_datas):
        domains = [int(generator.train_data[did][-1]) for did in cidxs]
        domain_counter = collections.Counter(domains)
        offset = 0
        y_bottom = row_map[cid] - client_height / 2.0
        y_top = row_map[cid] + client_height / 2.0
        for domain_idx in range(num_domains):
            plt.fill_between([offset, offset + domain_counter[domain_idx]], y_bottom, y_top,
                             facecolor=colors[domain_idx % len(colors)])
            offset += domain_counter[domain_idx]
    generator.train_data.set_mode('label')
    plt.title('Domain Heterogeneity')
    plt.xlim(0, max(data_columns))
    plt.ylim(-0.5, n - 0.5)
    plt.ylabel('Client ID')
    plt.xlabel('Number of Samples')
    plt.savefig(os.path.join(task_path, 'domain_heterogeneity.png'))
    plt.show()
