...
class OfficeCaltech10(torch.utils.data.ConcatDataset):
    ...
    test_domain_idx = np.random.choice(len(domains))
    test_domain = domains[test_domain_idx]
    train_domains = (domain for i, domain in enumerate(domains) if i != test_domain_idx)
    url = 'https://github.com/ChristophRaab/Office_Caltech_DA_Dataset/archive/refs/heads/main.zip'
    def __init__(self, root, split='train', download=True, transform=None, target_transform=None, transforms=None) -> None:
        datasets = []
        ...
        if split == 'train':
            ...
            datasets.append(OCDomainDataset(root, domain=self.test_domain, transform=transform, target_transform=target_transform, transforms=transforms))
...
train_data = OfficeCaltech10(path, 'train', transform=transform).datasets
test_data = OfficeCaltech10(path, 'test', transform=transform).datasets
