...
class PACS(torch.utils.data.ConcatDataset):
    ...
    def __init__(self, root, split='train', download=True, transform=None, target_transform=None, transforms=None) -> None:
        ...
        if split == 'train':
            ...
            datasets.append(PACSDomainDataset(root, domain=self.test_domain, transform=transform, target_transform=target_transform, transforms=transforms))
...
train_data = PACS(path, 'train', transform=transform).datasets
test_data = PACS(path, 'test', transform=transform).datasets
