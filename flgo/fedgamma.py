...
from flgo.utils.minimizers import LESAM
...
class Client(BasicClient):
    ...
    def train(self, model):
        # global parameters
        if self.paramL is None: perturb = - _model_to_tensor(model)
        else:  perturb = self.paramL - _model_to_tensor(model)
        perturb /= _modeldict_norm(perturb, p=2)

        model.train()
        optimizer = self.calculator.get_optimizer(model, lr=self.learning_rate, weight_decay=self.weight_decay, momentum=self.momentum)
        minimizer = LESAM(optimizer, self.model, self.rho)
