...
def hessian_eigs(
    ...
    """
    # Extract model parameters that are not biases
    params = [p for p in model.parameters() if len(p.shape) != 1]
    N = sum(p.numel() for p in params)  # Total number of parameters

    print(f"[INFO] Hessian will be computed for a {N}x{N} matrix.")  # Log total size of Hessian
    print(f"[INFO] Computing top {num_max} eigenvalues...")  # Log number of eigenvalues to compute

    # Define a function for Hessian-vector product
    ...
    # LinearOperator for Hessian computation
    A = LinearOperator((N, N), matvec=hess_vec_prod)

    # Compute the top `num_max` eigenvalues and eigenvectors
    w, v = eigsh(A, k=num_max, tol=1e-2)

    print(f"[INFO] Completed Hessian eigenvalue computation.")  # Log completion
    return w, v
...
def _eval_hess_vec_prod(vec, params, net, criterion, data_loader, device):
    ...
    # 包裹 data_loader，显示进度条
    progress_bar = tqdm(data_loader, desc="Evaluating Hessian-Vector Product")
    ...
    for x_batch, y_batch in progress_bar:
        ...
        # Compute the inner product of gradients and vector
        grad_times_vec = 0.0
        for g, v in zip(grad_f, vec):
            grad_times_vec += (g * v).sum()

        # Backward pass to accumulate Hessian-vector product
