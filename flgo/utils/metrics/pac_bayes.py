...
def estimate_accuracy(
    ...
    """
    Estimate the accuracy (or average loss) of the model on a given dataset.
    Optionally, limit the number of iterations with M3.

    Args:
        model: The model to evaluate.
        data_loader: DataLoader providing data to evaluate on.
        criterion: The loss function.
        device: The device to run evaluation on.
        M3: Optional[int], the number of iterations to limit evaluation to.
    ...
    # Choose whether to use M3 iterations or full dataset
    if M3 is None:
        model.eval()
        with torch.no_grad():
            # Wrap data_loader with tqdm to add a progress bar
            progress_bar = tqdm(data_loader, desc="Estimating Accuracy")
            for data in progress_bar:
                ...
                progress_bar.set_postfix({"Avg Loss": loss / total})
    else:
        model.eval()
        with torch.no_grad():
            # Wrap range(M3) with tqdm for limited iterations
            progress_bar = tqdm(range(M3), desc="Estimating Accuracy (M3)")
